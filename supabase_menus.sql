...
create table if not exists public.menus (
  ...
  label text not null,
  path text,
  icon text, -- Icon name string (e.g., 'Home', 'Users')
...
create policy "Public Select menus"
...
begin
  ...
  (parent_id, '작업일지', '/work-logs', 60, true, true), -- 작업일지는 모바일 허용 예시
  (parent_id, '롤 관리', '/roles', 70, true, false),
  (parent_id, '메뉴 관리', '/menus', 80, true, false), -- 새로 추가된 메뉴
  (parent_id, '접속 로그', '/access-logs', 90, true, false);
end $$;

-- 3. Device Management (Parent)
insert into public.menus (label, path, icon, "sortOrder", "isVisiblePc", "isVisibleMobile")
...
-- 4. Data Management (Parent)
insert into public.menus (label, path, icon, "sortOrder", "isVisiblePc", "isVisibleMobile")
values ('데이터 관리', null, 'Activity', 40, true, true); -- 데이터 관리는 모바일 허용 예시
